...
func File(suffix string) string {
	_, file, _, _ := runtime.Caller(0)
	fname := path.Base(file)
	fname = fname[:len(fname)-3]
	return "../../test_data/" + fname + suffix + ".js"
}
...
func printTest(t *testing.T, msg *big.Int, public kyber.Point, signature ethschnorr.Signature) {
	...
	*/
	...
	fname := path.Base(file)
	fname = fname[:len(fname)-3]
	fmt.Print("file name ", fname)
	//fil, err := os.Create(ethschnorr_test.File(""))
	...
	qwe := fmt.Sprintf("// This test was generated by ethdss_test.go, with printTests set to true\nconst %s = [['%064x',\n   '%064x',\n   '%064x',\n   '%064x',\n   '%040x']]\nmodule.exports = %s",
		fname,
		pX,
		pY,
		signature.Signature,
		...
		signature.CommitmentPublicAddress,
		fname)
	_ = ioutil.WriteFile("../../test_data/"+fname+".js", []byte(qwe[:]), 777)
	//require.Nil(t, err)
}
...
func TestDSSSignature(t *testing.T) {
	...
	assert.NotNil(t, sig)
	assert.Nil(t, err)
	assert.NoError(t, ethschnorr.Verify(longterms[1].Public(), dss0.msg, sig))
